...
function updateAllSectionToneOverlays() {
 ...
 if (control) {
  ...
  control.addEventListener("click", function (e) {
    ...
    // --- NEW LOGIC: repaint depending on whether sections are in sync ---
    if (!hasLocalChanges) {
      // All sections are effectively at the global tone:
      // repaint everything using the *current* globalIndex of the new family.
      applyGlobalAll();
    } else {
      // Sections have their own tones; keep their relative differences,
      // but remap those tones into the new family.
      applyLocal("header");
      applyLocal("hero");
      applyLocal("services");
      applyLocal("reviews");
 ...
 if (logo) {
  logo.addEventListener("pointerdown", function () {
    ...
    pressTimer = setTimeout(function () {
      ...
      updatePaletteIndexOverlay();
      updateAllSectionToneOverlays();  // <-- add this
      showMenu();
    }, 700);
  });
